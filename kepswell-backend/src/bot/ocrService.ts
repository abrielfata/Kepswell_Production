...
import ocrConfig from '../config/ocrConfig.json';
...
export class OCRService {
  private parseGMV(text: string): number {
    ...
    const patterns = ocrConfig.gmvPatterns.map((p) => new RegExp(p, 'i'));
  ...
  private parseDurationMinutes(text: string): number {
    const patterns = ocrConfig.durationPatterns.map((p) => ({
  ...
  private parsePesananSKU(text: string): number {
    ...
    const terjualMatch = clean.match(new RegExp(ocrConfig.pesananSkuPatterns.terjualMatch, 'i'));
    if (terjualMatch) return parseInt(terjualMatch[1], 10);

    // Prioritas 1: GMV dengan suffix K diikuti LANGSUNG angka
    const afterGmvK = clean.match(new RegExp(ocrConfig.pesananSkuPatterns.afterGmvK, 'i'));
    ...
    const afterGmvNoK = clean.match(new RegExp(ocrConfig.pesananSkuPatterns.afterGmvNoK, 'i'));
    if (afterGmvNoK) return parseInt(afterGmvNoK[1], 10);

    // Prioritas 3: Angka yang langsung SEBELUM label "Pesanan SK..."
    const beforeLabel = clean.match(new RegExp(ocrConfig.pesananSkuPatterns.beforeLabel, 'i'));
    if (beforeLabel) return parseInt(beforeLabel[1], 10);

    // Prioritas 4: Angka setelah label "Pesanan SK"
    const afterLabel = clean.match(new RegExp(ocrConfig.pesananSkuPatterns.afterLabel, 'i'));
  ...
  private parseLiveDate(text: string): string | null {
    ...
    const sanitizedText = text.replace(
      new RegExp(ocrConfig.liveDatePatterns.timeFix, 'g'),
      (match) => {
        return match
          .replace(/[OoQ]/g, '0')
          .replace(/[Iil|]/g, '1')
    ...
    const relativeMatch = sanitizedText.match(
      new RegExp(ocrConfig.liveDatePatterns.relativeMatch, 'i'),
    );
    if (relativeMatch) {
    ...
    const tiktokRangeMatch = sanitizedText.match(
      new RegExp(ocrConfig.liveDatePatterns.tiktokRangeMatch, 'i'),
    );
    if (tiktokRangeMatch) {
    ...
    const numericMatch = sanitizedText.match(new RegExp(ocrConfig.liveDatePatterns.numericMatch));
    if (numericMatch) {
    ...
    const textMatch = sanitizedText.match(new RegExp(ocrConfig.liveDatePatterns.textMatch, 'i'));
    if (textMatch) {
