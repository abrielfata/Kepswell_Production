...
export class HostRepository {
  ...
  async registrationCodeExists(code: string): Promise<boolean> {
    const result = await query('SELECT 1 FROM host_registration_codes WHERE code = $1 LIMIT 1', [
      code,
    ]);
    return (result.rowCount ?? 0) > 0;
  }
  ...
  async insertRegistrationCode(hostId: number, code: string): Promise<void> {
    await query(`INSERT INTO host_registration_codes (host_id, code) VALUES ($1, $2)`, [
      hostId,
      code,
    ]);
  }
  ...
  async deleteUnusedRegistrationCodesForHost(hostId: number): Promise<void> {
    await query('DELETE FROM host_registration_codes WHERE host_id = $1 AND used_at IS NULL', [
      hostId,
    ]);
  }
  ...
  async updateChatIdByCode(
    ...
    return withTransaction(async (client) => {
      ...
      const taken = await client.query('SELECT id FROM hosts WHERE telegram_chat_id_hash = $1', [
        hash,
      ]);
